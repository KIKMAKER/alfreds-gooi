...
9. **Never use `button_to` inside a `form_with` block** - `button_to` renders its own `<form>` tag, creating invalid nested HTML. Browsers handle nested forms inconsistently and can corrupt the outer form's `authenticity_token`, causing CSRF 422 errors. Use `link_to` with `data: { turbo_method: :post }` instead — it submits via Turbo fetch and reads CSRF from the meta tag without generating a form element.
10. **Sanitise query params before assigning to enum attributes** - Rails enum raises `ArgumentError` at assignment time (not validation time), so passing a raw query param like `params[:plan]` directly into `Model.new(plan: params[:plan])` will 500 if the value is invalid (e.g. from a bot probe). Always validate first: `Model.plans.key?(params[:plan]) ? params[:plan] : nil`.
11. **Stimulus `data-action` descriptors look like URL paths to bots** - Security scanners read `data-action="submit->controller#method"` off the HTML and POST to `/path/submit->controller` as if it were a URL. These will appear as 404s in logs but are harmless — no code fix needed, just noise to recognise.
12. **Never add a text-based `gooi*` wordmark to inline SVGs rendered on the website** - The real Gooi logo lives in the nav bar (and in standalone SVG files produced in Affinity Designer). Adding `<text>gooi*</text>` as a logo substitute inside inline SVG slides/decks is incorrect and should not be done. If a deck slide needs a logo, leave a clearly-labelled placeholder rect and note it must be replaced with the Affinity-exported vector path.
