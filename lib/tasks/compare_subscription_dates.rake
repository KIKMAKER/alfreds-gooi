namespace :data do
  desc "Compare subscription dates from CSV with the database (outputs to console for Heroku)"

  task compare_subscription_dates: :environment do
    input_file_path = Rails.root.join("lib", "assets", "subscription_dates.csv")

    puts "customer_id,csv_start_date,db_start_date,csv_duration,db_duration,status"
    ...
    CSV.foreach(input_file_path, headers: true) do |row|
      ...
      if user.nil?
        puts [customer_id, csv_start_date, nil, csv_duration, nil, "User not found"].to_csv
        next
      end
      ...
      if latest_subscription.nil?
        puts [customer_id, csv_start_date, nil, csv_duration, nil, "No subscriptions found"].to_csv
        next
      end
      ...
      puts [customer_id, csv_start_date, db_start_date, csv_duration, db_duration, status].to_csv
    end
  end
end
